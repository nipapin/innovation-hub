...
13. **Вход в справку — иконка сразу после заголовка**, без подписи, не своей вёрсткой и не в другом месте шапки. Уровней три, и путать их нельзя: у названия инструмента — `help` у `AdminPageHeader` / `AccountPageHeader`, с чипом `F1`; у заголовка секции — `<HelpSectionButton>`, та же иконка, но **без F1 и без чипа** (клавиша обязана быть однозначной, а секций на странице несколько); у отдельного параметра — знак «?» (`<HelpDot>`) вплотную к нему. Кнопка у секции ставится, только когда у секции есть **своя** статья: ведущая на статью страницы обещает подробности, которых нет. Заголовок диалога считается заголовком страницы, а не параметром. Полный контракт — [HELP_SYSTEM.md](./HELP_SYSTEM.md).
...
Хардкод цвета в кабинете закрыт 2026-09-11 (174 вхождения переведены на токены),
и назад не вернётся: `npm run color:check` валит сборку на литеральном цвете в
классе. Осознанные исключения — в [UI_TOKENS.md §11](./UI_TOKENS.md#11-таблица-миграции-хардкод--токен).
