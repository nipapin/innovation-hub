...
export type ExposedOption = {
  ...
  siblingKey: string
  /**
   * `data.pluginId` ноды-владельца: `checkFolder`, `mainSearch` и прочие.
   *
   * Не для показа — ноды клиенту не показываются и показываться не будут
   * (docs/PROJECT_OPTIONS_PANEL.md §1). Единственное назначение: у ноды может
   * быть СВОЙ интерфейс на сайте, и тогда её свойства не должны появляться ещё
   * и общим списком настроек. Так у `checkFolder`: её требования и шаблон имени
   * рисует диалог сборки элемента (docs/TOOLS_FOLDER_ASSEMBLY_PLAN.md).
   *
   * `null` — свойство разобрали вне графа (так делает запись, apply.ts): там
   * корень не при чём, а поле нужно только чтению.
   */
  nodePlugin: string | null
  /**
