...
export const EXPOSED_CONTROL_TYPES = [
  ...
  "vendorAccount",
  /**
   * Выбор файла (`PathNavigator.tsx` в программе). В значении — путь
   * относительно папки проекта, и плагин резолвит его сам
   * (`path.join(_description.projectPathGD, rawPath)`), поэтому со стороны
   * обработки менять не пришлось ничего.
   *
   * Клиент не «указывает путь», а ГРУЗИТ свой файл: он лежит у него на
   * компьютере, а не на машине парка. Файл уходит в `Assets/`, в значение
   * пишется `Assets/<имя>`.
   *
   * Фильтра по расширению здесь нет намеренно: в ноде программа зовёт
   * `selectFiles` без `filters`, и фильтр на сайте сделал бы его строже
   * программы — то же действие, разрешённое на машине и запрещённое здесь.
   */
  "pathNavigator",
  /**
   * Положение логотипа поверх кадра (`OverlayEdit` в программе) —
   * docs/OVERLAY_CONTROL_PLAN.md.
   *
   * Единственный контрол, у которого значение — СТРОКА с JSON, а не данные:
   * так это лежит в `options.json` сегодня, плагин делает `JSON.parse`, и
   * менять форму ради удобства сайта нельзя — читает её не только сайт.
   *
   * Сайт правит ТОЛЬКО геометрию трёх форматов. Всё остальное внутри значения
   * (`encode`, `fgFilePath`, `bgFilePath`) принадлежит программе и возвращается
   * нетронутым — слияние в apply.ts, а не доверие браузеру.
   */
  "overlaySettings",
  /**
   * Смена формата кадра (`VideoAdjustEdit` в программе) —
   * docs/VIDEO_ADJUST_CONTROL_PLAN.md. Вертикальный ролик из горизонтального:
   * кадр вписывается в новый формат, пустое место занимает тот же ролик,
   * размытый и подкрашенный.
   *
   * Значение — снова СТРОКА с JSON. Сайт правит шесть полей и блок тени;
   * `encode`, `finalFormat`, `copies`, `bgColor` и пути превью принадлежат
...
export type ExposedOption = {
  ...
  siblingKey: string
  /**
   * Адрес файла, который реально будет наложен — только у `overlaySettings`
   * (docs/OVERLAY_CONTROL_PLAN.md §8.5). Заполняется НЕ разбором графа: путь
   * оттуда, а ссылку на объект собирает слой хранилища, знающий ключи.
   *
   * `null` — источник не найден; рамка в модалке остаётся пустой.
   */
