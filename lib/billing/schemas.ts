...
export const trialWriteSchema = z.object({
  ...
  /**
   * Состав пробного набора целиком, в порядке показа.
   *
   * Необязательный: настройки периода и набор сохраняются двумя разными
   * кнопками, и запрос без этого поля не трогает проекты вовсе. Отличать
   * «поле не передали» от «набор осознанно опустошили» обязательно — иначе
   * сохранение суммы подарка стирало бы набор.
   *
   * Список целиком, а не diff: состав — одно распоряжение, и присланный
   * порядок становится порядком копирования.
   */
