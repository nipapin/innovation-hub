 ...
 *
 * Правило действует и для ПАПОК, и для ФАЙЛОВ — с 2026-09-23.
 *
 * Раньше оно было только для папок: у файла дефис в начале имени считался просто
 * дефисом. Это перестало быть верным, когда десктоп сделал пометку дефисом
 * ЕДИНСТВЕННЫМ обращением с браком (`markSkippedWithDash` в processItem.ts) —
 * и для упавшей обработки, и для исходника, который не удалось удалить. Такой
 * файл остаётся лежать в IN, и пока правило не распространялось на файлы, обе
 * линии сборки заводили по нему задачу снова: пометка не значила ничего.
 *
 * Обратная дорога поэтому тоже одна на всех: снять `-` с имени. Для папки это
 * «доложил файлы, забирай», для файла — «разобрался с ошибкой, пробуй ещё раз».
 * Снятие приезжает событием `move` внутри IN, и линия событий заводит задачу.
 */
...
export async function collectTasks(): Promise<CollectResult> {
  ...
  for (const change of changes.rows) {
    ...
    // ИЛИ ФАЙЛА приезжает именно как move, и раньше это событие выбрасывалось
    // вместе с остальными, из-за чего готовая папка так и не попадала в очередь.
    // Для файла это ещё и единственный способ вернуть в работу то, что упало.
     ...
     *
     * Проверка осталась для переносов, которые делает ЧЕЛОВЕК: унёс элемент из
     * IN — работы это не создаёт.
     *
     * Карантин упавшей задачи через неё больше не проходит: с 2026-09-23 он не
     * переносит исходник, а помечает дефисом на месте (lib/pipeline/quarantine.ts),
     * и от самоотмены его спасает `isHeldBack` ниже. Раньше страховка была именно
     * здесь: уникальный индекс не помеха, он держится только на живых статусах, а
     * старая задача упала, — и перенос в папку ошибок сам заводил новую задачу.
     */
    if (change.op === "move") {
