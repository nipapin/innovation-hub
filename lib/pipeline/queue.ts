...
export async function failTask(input: {
  ...
  // Исходник помечаем дефисом, оставляя в IN. `failed` — состояние терминальное:
  // назад в очередь эту задачу никто не вернёт, а пока файл лежит в IN без
  // пометки, он для конвейера невидим (обход берёт только элементы без задач).
  // Пометка делает это видимым глазом и оставляет дорогу назад — снять дефис.
...
export async function reapExpiredLeases(): Promise<number> {
  ...
  // `taskFailed`, только сказать о нём было некому. Исходник помечаем по тому же
