  ...
  if (menu.kind === "file" && menu.file && inTrashPane) {
    ...
    entries = [
      ...
      ...(!many && file.isFolder
        ...
        : []),
      /*
        «Править элемент» — для папки элемента в IN. То же, что обычный клик по
        такой папке; пункт остаётся, потому что меню — привычная дорога, а по
        клику это действие ожидают не все: в остальных папках клик заходит
        внутрь. Внутрь отсюда ведёт иконка на строке.
      */
