...
/**
 * Что сейчас едет наверх. Пачка заливается по одному файлу, поэтому состояние
 * одно на всю область: кольцо показывает текущий файл, а `index` / `total`
 * говорят, сколько ещё за ним.
 */
export type UploadProgress = {
  /**
   * Папка, в которую льём, — логический путь от корня проекта. По нему область
   * файлов узнаёт, что кольцо её: заливка идёт в одну папку, а областей на
   * экране несколько (колонки, панели IN / OUT, мобильный список).
   */
  folderPath: string
  /** Имя текущего файла — то, под которым он ляжет. */
  name: string
  /** Какой по счёту в пачке, с единицы. */
  index: number
  total: number
  /** Проценты текущего файла, целые. */
  percent: number
}
